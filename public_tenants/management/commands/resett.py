...
class Command(ResetCommand):
    ...
    def handle(self, *args, **kwargs):
        try:
            ...
            tenants_list = []
            try:
                query = "select id,name from public_tenants_tenant"
                tenants_tuple = self.exec_query_on_default(default_config, query, 1)

                for obj in tenants_tuple:
                    tenant_id_str = str(obj[0])
                    query = "select name from public_tenants_tenantapp where tenant_id="+tenant_id_str
                ...
                tenant_table_error = 'relation "public_tenants_tenant" does not exist' in error_message
                tenant_app_table_error = 'relation "public_tenants_tenant_app" does not exist' in error_message
            ...
            for tenant_dict_obj in tenants_list:
                ...
                tenant_apps = []# tobe tenant_dict_obj['apps']
